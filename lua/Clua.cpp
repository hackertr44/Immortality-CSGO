...
namespace ns_client
{
	...
	{		
		...
		{
			eventlogs::get().addnew(crypt_str("Lua error: invalid callback \"") + eventname + '\"');
			return;
		}
...
namespace ns_menu
{
		...
		if (config_items.find(name) == config_items.end())
		{
			...
			for (auto item : cfg_manager->items)
			{
				if (item->name == name)
				{
					if (item->type != type)
					{
						eventlogs::get().addnew(crypt_str("Lua error: invalid config item type, must be ") + type);
						return false;
					}
			...
			if (!found)
			{
				eventlogs::get().addnew(crypt_str("Lua error: cannot find config variable \"") + name + '\"');
				return false;
			}
		}
	...
	bool get_bool(std::string name)
	{
		...
		if (it.type == NEXT_LINE)
		{
			...
			eventlogs::get().addnew(crypt_str("Lua error: cannot find menu variable \"") + name + '\"');
			return false;
		}
...
namespace ns_console
{
	...
	bool get_bool(const std::string& convar_name)
	{
		if (convars.find(convar_name) == convars.end())
		{
			...
			{
				eventlogs::get().addnew(crypt_str("Lua error: cannot find ConVar \"") + convar_name + '\"');
				return false;
			}
		}
...
namespace ns_file //new
{
	void append(sol::this_state s, std::string& path, std::string& data)
	{
		if (!g_cfg.scripts.allow_file)
		{
			...
			eventlogs::get().addnew(crypt_str("Please, allow files read or write"));
			return;
		}
		...
		if (out.is_open())
			...
			eventlogs::get().addnew(crypt_str("Can't append to file: ") + path);
	...
	void write(sol::this_state s, std::string& path, std::string& data)
	{
		...
		if (out.is_open())
			...
			eventlogs::get().addnew(crypt_str("Can't write to file: ") + path);
	...
	std::string read(sol::this_state s, std::string& path)
	{
		...
		{
			eventlogs::get().addnew(crypt_str("Can't read file: ") + path);
			file.close();
			return "";
		}
	}
