...
std::string get_wep(int id, int custom_index = -1, bool knife = true)
{
	if (custom_index > -1)
	{
		...
		else
		{
			switch (custom_index)
			{
			case 0: return crypt_str("ct_gloves"); //-V1033
...
void AddKeybind(std::string Bind, std::string State)
{
	ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 6); ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 3); ImGui::Text(Bind.c_str());
	ImGui::SameLine();
	ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 7); ImGui::SetCursorPosX(187 - ImGui::CalcTextSize(State.c_str()).x); ImGui::Text(State.c_str());
}
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\whiteless.us\\scripts\\");
...
namespace ImGui
{
	...
	bool Tab(const char* icon, const char* label, const ImVec2& size_arg, const bool selected)
	{
		...
		if (selected) {
			window->DrawList->AddRectFilled({ bb.Min.x + 1, bb.Min.y - 1 }, { bb.Max.x - 1,bb.Max.y + 1 }, ImColor(32, 32, 32));
			window->DrawList->AddRectFilled({ bb.Min.x - 1, bb.Min.y + 1 }, { bb.Max.x + 1,bb.Max.y - 1 }, ImColor(32, 32, 32));
		}

		ImGui::PushFont(c_menu::get().super_ico);
		window->DrawList->AddText(ImVec2(bb.Min.x + size_arg.x / 4, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(icon).y / 2), ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b()), icon);
		ImGui::PopFont();
	...
	bool SubTab(const char* label, const ImVec2& size_arg, const bool selected)
	{
		...
		if (selected)
			window->DrawList->AddRectFilled({ bb.Min.x,bb.Min.y }, { bb.Max.x,bb.Max.y }, ImColor(23, 23, 23, 255));

		//ImGui::PushFont(c_menu::get().bigxd);
		window->DrawList->AddText(ImVec2(bb.Min.x + size_arg.x / 2 - ImGui::CalcTextSize(label).x / 2, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(label).y / 2), ImColor(255, 255, 255), label);
		//ImGui::PopFont();
...
// IMPORTANT: DO TAB SWITCHING BY LAST_TAB!!!!!
	...
	if (ImGui::Begin(crypt_str("whiteless"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
	{
		...
		if (g_cfg.menu.background_line)
		{

		}

		ImGui::BeginChild("##Full", ImVec2{ 650, 820 }, false);
		{
			Style->Colors[ImGuiCol_ChildBg] = ImColor(20, 20, 20);
			ImGui::BeginChild("##Up", ImVec2{ 50, 820 }, false);
			{
				ImGui::SetCursorPos(ImVec2{ 12, 12 });
				ImGui::PushFont(c_menu::get().neverpuk_large); ImGui::TextColored(ImColor(240, 240, 240, 255), "WL"); ImGui::PopFont();
				//ImGui::SameLine(); ImGui::Image(yourlogo, ImVec2(85 * dpi_scale, 85 * dpi_scale));
				//ImGui::Image(logotype, ImVec2(40.0f, 40.0f)); //Когда сделать картинку в байты тогда раскомменти, а то так краш будет



#if BETA
				ImGui::SetCursorPos(ImVec2{ 90, 25 });
				ImGui::PushFont(c_menu::get().neverpuk_large); ImGui::TextColored(ImColor(255, 255, 255), ""); ImGui::PopFont();
#endif
				ImGui::SetCursorPos(ImVec2{ 0, 50 });
				if (ImGui::Tab("A", "rage", ImVec2(50, 50), Active_Tab == 1 ? true : false))
					Active_Tab = 1;

				ImGui::SetCursorPos(ImVec2{ 0, 100 });
				if (ImGui::Tab("B", "antiaim", ImVec2(50, 50), Active_Tab == 6 ? true : false))
					Active_Tab = 6;

				ImGui::SetCursorPos(ImVec2{ 0, 150 });
				if (ImGui::Tab("H", "legit", ImVec2{ 50, 50 }, Active_Tab == 2 ? true : false))
					Active_Tab = 2;

				ImGui::SetCursorPos(ImVec2{ 0, 200 });
				if (ImGui::Tab("C", "visuals", ImVec2{ 50, 50 }, Active_Tab == 3 ? true : false))
					Active_Tab = 3;

				ImGui::SetCursorPos(ImVec2{ 0, 250 });
				if (ImGui::Tab("D", "player", ImVec2{ 50, 50 }, Active_Tab == 4 ? true : false))
					Active_Tab = 4;

				ImGui::SetCursorPos(ImVec2{ 0, 300 });
				if (ImGui::Tab("E", "misc", ImVec2{ 50, 50 }, Active_Tab == 5 ? true : false))
					Active_Tab = 5;

				ImGui::SetCursorPos(ImVec2{ 0, 350 });
				if (ImGui::Tab("F", "cfg", ImVec2{ 50, 50 }, Active_Tab == 8 ? true : false))
					Active_Tab = 8;

				ImGui::SetCursorPos(ImVec2{ 0, 400 });
				if (ImGui::Tab("G", "lua", ImVec2{ 50, 50 }, Active_Tab == 7 ? true : false))
					Active_Tab = 7;

				ImGui::SetCursorPos(ImVec2{ 5, 460 });
				if (ImGui::CustomButton(crypt_str("Save"), crypt_str("##save_all"), ImVec2(40, 25)))
				{
					save_config();
				}
			}
			...
			Style->ChildRounding = 0;
			ImGui::BeginChild("##Draw Function", ImVec2{ 610, 820 }, false, NoScroll);
			{
				ImGui::SetCursorPos(ImVec2{ 0, 0 });
				ImGui::BeginChild("##Function", ImVec2{ 610, 820 }, false), NoScroll;
				{
					Style->Colors[ImGuiCol_ChildBg] = ImColor(23, 23, 23);
					if (Active_Tab == 6) {
						static auto type = 0;
						ImGui::SetCursorPos(ImVec2{ 20, 20 });
						ImGui::BeginChild("antiaim", { 270, 170 }); {

							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("antiaim", { 250, 150 }); {
						...
						ImGui::BeginChild("antiaim settings", { 270, 150 });
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("antiaim", { 250, 130 });
							{
								ImGui::TabName(crypt_str("Misc"));
						...
						ImGui::BeginChild("antiaim fakelag", { 270, 160 });
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("fakelag", { 250, 140 });
							{
						...
						ImGui::SetCursorPos(ImVec2{ 310, 20 });
						ImGui::BeginChild("antiaim general", { 270, 520 });
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("antiaim", { 250, 500 });
							{
					...
					else if (Active_Tab == 1) //Rage | General
					{
						...
						ImGui::BeginChild("##Main", ImVec2{ 560, 30 }, false, NoMove);
						{
							ImGui::SetCursorPos(ImVec2{ 0, 0 });
							if (ImGui::SubTab("General", ImVec2(270, 30), Active_Rage_Tab == 1 ? true : false))
								Active_Rage_Tab = 1;

							ImGui::SetCursorPos(ImVec2{ 290, 0 });
							if (ImGui::SubTab("Accuracy", ImVec2(270, 30), Active_Rage_Tab == 2 ? true : false))
								Active_Rage_Tab = 2;
						}
						ImGui::EndChild();
						...
						if (Active_Rage_Tab == 1)
						{
							...
							ImGui::BeginChild("1", { 270, 420 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("1", { 250, 400 });
								{
									ImGui::TabName(crypt_str("General"));
							...
							ImGui::BeginChild("2", { 270, 250 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("2", { 250, 230 });
								{
						...
						{
							ImGui::SetCursorPos(ImVec2{ 20, 50 });
							ImGui::BeginChild("1", { 270, 520 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("1", { 250, 500 });
								{
									ImGui::TabName(crypt_str("General"));
							...
							ImGui::SetCursorPos(ImVec2{ 310, 50 });
							ImGui::BeginChild("2", { 270, 520 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("2", { 250, 500 });
								{
					...
					if (Active_Tab == 2) //Legit | General
					{
						...
						ImGui::SetCursorPos(ImVec2{ 20, 20 });
						ImGui::BeginChild("legit bot", { 270, 70 }); 
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("legit bot", { 250, 50 }); 
							{
								ImGui::TabName(crypt_str("General"));
						...
						ImGui::BeginChild("legit bot settings", { 270, 250 }); 
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("legit bot", { 250, 230 }); 
							{
								ImGui::TabName(crypt_str("Settings"));
						...
						ImGui::SetCursorPos(ImVec2{ 20, 380 });
						ImGui::BeginChild("legit bot exploits", { 270, 160 }); 
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("legit bot", { 250, 140 }); 
							{
								ImGui::TabName(crypt_str("Rcs"));
						...
						ImGui::SetCursorPos(ImVec2{ 310, 20 });
						ImGui::BeginChild("legit bot select weapon", { 270, 70 }); 
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("legit bot", { 250, 50 }); 
							{
						...
						ImGui::SetCursorPos(ImVec2{ 310, 110 });
						ImGui::BeginChild("legit bot weapon", { 270, 340 }); 
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("legit bot", { 250, 320 }); 
							{
								ImGui::TabName(crypt_str("Settings"));
						...
						ImGui::SetCursorPos(ImVec2{ 310, 470 });
						ImGui::BeginChild("legit bot accuracy", { 270, 70 }); 
						{
							ImGui::SetCursorPos(ImVec2{ 10, 10 });
							ImGui::BeginChild("legit bot", { 250, 50 }); 
							{
					...
					if (Active_Tab == 3) //Visuals | ESP
					{
						...
						{
							ImGui::SetCursorPos(ImVec2{ 0, 0 });
							if (ImGui::SubTab("General", ImVec2(270, 30), Active_Visual_Tab == 1 ? true : false))
								Active_Visual_Tab = 1;

							ImGui::SetCursorPos(ImVec2{ 290, 0 });
							if (ImGui::SubTab("World", ImVec2(270, 30), Active_Visual_Tab == 2 ? true : false))
								Active_Visual_Tab = 2;
						}
						ImGui::EndChild();

						Style->Colors[ImGuiCol_ChildBg] = ImColor(23, 23, 23);
						if (Active_Visual_Tab == 1)
						{
							ImGui::SetCursorPos(ImVec2{ 20, 50 });
							ImGui::BeginChild("1", { 270, 520 }); {
							...
							ImGui::SetCursorPos(ImVec2{ 310, 50 });
							ImGui::BeginChild("2", { 270, 420 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("2", { 250, 400 });
								{
						...
						else if (Active_Visual_Tab == 2) {
							ImGui::SetCursorPos(ImVec2{ 20, 50 });
							ImGui::BeginChild("1", { 270, 230 }); {

								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("1", { 250, 210 }); {
							...
							ImGui::SetCursorPos(ImVec2{ 310, 50 });
							ImGui::BeginChild("2", { 270, 230 }); {

								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("2", { 250, 210 }); {
							...
							ImGui::BeginChild("3", { 270, 240 }); {

								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("3", { 250, 220 }); {
							...
							ImGui::SetCursorPos(ImVec2{ 310, 300 });
							ImGui::BeginChild("4", { 270, 240 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("4", { 250, 220 });
								{
					...
					if (Active_Tab == 4) // player chams 
					{
						...
						{
							ImGui::SetCursorPos(ImVec2{ 0, 0 });
							if (ImGui::SubTab("Chams", ImVec2(270, 30), Active_changer_Tab == 1 ? true : false))
								Active_changer_Tab = 1;

							ImGui::SetCursorPos(ImVec2{ 290, 0 });
							if (ImGui::SubTab("Changer", ImVec2(270, 30), Active_changer_Tab == 2 ? true : false))
								Active_changer_Tab = 2;
						}
						ImGui::EndChild();

						Style->Colors[ImGuiCol_ChildBg] = ImColor(23, 23, 23);
						if (Active_changer_Tab == 1)
						{
							ImGui::SetCursorPos(ImVec2{ 20, 50 });
							ImGui::BeginChild("1", { 270, 520 });
							{
						...
						else if (Active_changer_Tab == 2)
						{
							...
							ImGui::SetCursorPos(ImVec2{ 20, 50 });
							ImGui::BeginChild(crypt_str("##SKINCHANGER__CHILD"), ImVec2(580 * dpi_scale, (child_height - 108) * dpi_scale));
							{
								...
								else
								{
									...
									if (ImGui::Combo(crypt_str("##Quality_combo"), &selected_entry.entity_quality_vector_index, [](void* data, int idx, const char** out_text)
										...
										if (ImGui::CustomButton(crypt_str("Close"), crypt_str("##CLOSE__SKING"), ImVec2(198 * dpi_scale, 40 * dpi_scale)))
										{
											// start animation
											active_animation = true;
											next_id = -1;
					...
					if (Active_Tab == 5) //Misc | Main
					{
						...
						{
							ImGui::SetCursorPos(ImVec2{ 0, 0 });
							if (ImGui::SubTab("General", ImVec2(270, 30), Active_Misc_Tab == 1 ? true : false))
								Active_Misc_Tab = 1;

							ImGui::SetCursorPos(ImVec2{ 290, 0 });
							if (ImGui::SubTab("Additives", ImVec2(270, 30), Active_Misc_Tab == 2 ? true : false))
								Active_Misc_Tab = 2;
						}
						ImGui::EndChild();
						...
						else if (Active_Misc_Tab == 2)
						{
							ImGui::SetCursorPos(ImVec2{ 20, 50 });
							ImGui::BeginChild("19", { 270, 150 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("19", { 250, 130 });
								{
									ImGui::TabName(crypt_str("Buttons"));

									if (ImGui::CustomButton(crypt_str("unlock hidden convars"), crypt_str("##GAME__UNLOCK"), ImVec2(230 * dpi_scale, 21 * dpi_scale)))
										misc::get().EnableHiddenCVars();

									if (ImGui::CustomButton(crypt_str("rage quit"), crypt_str("##GAME__QU1T"), ImVec2(230 * dpi_scale, 21 * dpi_scale)))
										fRageQuit();
								}
								ImGui::EndChild();
							}
							...
							ImGui::SetCursorPos(ImVec2{ 20, 210 });
							ImGui::BeginChild("23", { 270, 150 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("23", { 250, 130 });
								{
									ImGui::TabName(crypt_str("Render menu"));
							...
							ImGui::SetCursorPos(ImVec2{ 310, 50 });
							ImGui::BeginChild("20", { 270, 280 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("20", { 250, 260 });
								{
									ImGui::TabName(crypt_str("Other"));
							...
							ImGui::BeginChild("21", { 270, 150 });
							{
								ImGui::SetCursorPos(ImVec2{ 10, 10 });
								ImGui::BeginChild("21", { 250, 130 });
								{
				...
				if (Active_Tab == 8) // Configs | Main
				{
					...
					ImGui::BeginChild("2", { 270, 520 });
					{
						...
						ImGui::BeginChild("2", { 250, 500 });
						{
							ImGui::TabName(crypt_str(u8"Coming soon..."));
				...
				if (Active_Tab == 7) // Lua | Main
				{
					...
					ImGui::BeginChild("2", { 270, 520 });
					{
						...
						ImGui::BeginChild("2", { 250, 500 });
						{
							ImGui::TextColored(ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255), crypt_str("Script elements"));
...
void c_menu::create_style()
{
	...
	_style.Colors[ImGuiCol_::ImGuiCol_ChildBg] = ImColor(30, 30, 30, 255);
...
{
	const char* rage_sections[2] = { crypt_str("Aimbot"), crypt_str("Accuracy") };
...
void c_menu::draw_ragebot(int child)
{
	if (!rage_section)
	{
		if (!child)
		{
			child_title(crypt_str("Aimbot"));

			tab_start();
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				ImGui::Spacing();
				...
				{
					ImGui::SameLine();
					draw_keybind(crypt_str(""), &g_cfg.ragebot.enable_key, crypt_str("##HOTKEY_ENABLEKEY"));
				}
				...
				ImGui::Checkbox(crypt_str("Automatic scope"), &g_cfg.ragebot.autoscope);
				ImGui::Checkbox(crypt_str("Pitch correction"), &g_cfg.ragebot.pitch_correction);
				ImGui::Checkbox(crypt_str("Desync resolver"), &g_cfg.ragebot.resolver);
			}
			ImGui::EndChild();
			tab_end();
		}
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Ignore limbs when moving"), &g_cfg.ragebot.rage_aimbot_ignore_limbs);
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				draw_combo(crypt_str("Body conditions"), g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaim_conditions, baim_cond, ARRAYSIZE(baim_cond));
				if (g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaim_conditions == 1)
				{
					ImGui::SliderInt(crypt_str("Baim when health"), &g_cfg.ragebot.weapon[hooks::rage_weapon].body_aim_health, 10, 100);
				}
				...
				if (g_cfg.ragebot.double_tap && hooks::rage_weapon <= 4)
				{
					...
					//draw_combo(crypt_str("Double tap modifiers"), g_cfg.ragebot.dt_modifiers, dt_modeifiers, ARRAYSIZE(dt_modeifiers));
				}
...
void c_menu::draw_tabs_antiaim()
{
	const char* aa_sections[2] = { crypt_str("General"), crypt_str("Fakelag") };
	draw_semitabs(aa_sections, 2, antiaim_section, ImGui::GetStyle());
}

void c_menu::draw_antiaim(int child)
{
	if (!child)
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ANTIAIM1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			else
			{
				...
				if (g_cfg.antiaim.type[type].desync)
				{
					...
					{
						ImGui::SliderInt(crypt_str("Switch speed"), &g_cfg.antiaim.type[type].spin_speed, 1, 15);
						ImGui::SliderInt(crypt_str("Desync switch range"), &g_cfg.antiaim.type[type].desync_spin_range, 1, 60);
						ImGui::SliderInt(crypt_str("Inverted desync switch range"), &g_cfg.antiaim.type[type].inverted_desync_spin_range, 1, 60);
					}
	...
	{
		tab_start();
		ImGui::BeginChild(crypt_str("##ANTIAIMG2_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			draw_keybind(crypt_str("Anti-aim forward"), &g_cfg.antiaim.manual_forward, crypt_str("##HOTKEY_INVERT_FORWARD"));
			...
			draw_combo(crypt_str("Movement type"), g_cfg.misc.walktype, movetypes, ARRAYSIZE(movetypes));
	...
	if (visuals_section != 2)
	{
		if (!child)
		{
			...
			else
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL2_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
				{
		...
		else
		{
			if (!visuals_section)
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL1_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					ImGui::Checkbox(crypt_str("Rain"), &g_cfg.esp.rain);
					ImGui::Checkbox(crypt_str("Full bright"), &g_cfg.esp.bright);
			...
			else
			{
				...
				tab_start();
				ImGui::BeginChild(crypt_str("##VISUAL2_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					ImGui::Checkbox(crypt_str("Arms chams"), &g_cfg.esp.arms_chams);
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##armscolor"), &g_cfg.esp.arms_chams_color, ALPHA);
...
void c_menu::draw_players(int child)
{
	...
	if (!child)
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
	...
	else
	{
		...
		tab_start();
		ImGui::BeginChild(crypt_str("##ESP2_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			ImGui::Spacing();
			if (player == LOCAL)
				draw_combo(crypt_str("Type"), g_cfg.player.local_chams_type, local_chams_type, ARRAYSIZE(local_chams_type));
...
void c_menu::draw_misc(int child)
{
	if (!child)
	{
		if (!misc_section)
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##MISC1_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				ImGui::Checkbox(crypt_str("Anti-screenshot"), &g_cfg.misc.anti_screenshot);
				ImGui::Checkbox(crypt_str("Anti-untrusted"), &g_cfg.misc.anti_untrusted);
				...
				if (ImGui::CustomButton(crypt_str("RAGE QUIT"), crypt_str("##GAME__QU1T"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
					fRageQuit();

				if (ImGui::CustomButton(("Unlock hidden convars"), crypt_str("##GAME__UNLOCK"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
					misc::get().EnableHiddenCVars();
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				ImGui::Checkbox(crypt_str("Watermark"), &g_cfg.menu.watermark);
				ImGui::Checkbox(crypt_str("Spectators"), &g_cfg.misc.spectators_list);
				ImGui::Checkbox(crypt_str("Keybind list"), &g_cfg.esp.keybinds);
				...
				{
					ImGui::Text(crypt_str("Logs Gradient Color #1 "));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##logcolor"), &g_cfg.misc.log_color, ALPHA);
					ImGui::Text(crypt_str("Logs Gradient Color #2 "));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##logcolor2"), &g_cfg.misc.log_color2, ALPHA);
				}
	...
	else
	{
		if (!misc_section)
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
		...
		else
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##MISC2_SECOND"), ImVec2(358 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				ImGui::Checkbox(crypt_str("Grenade helper"), &g_cfg.misc.grenade_helper);
				ImGui::Checkbox(crypt_str("Force immunity"), &g_cfg.misc.immunity);
				ImGui::Checkbox(crypt_str("Clantag Spammer"), &g_cfg.misc.clantag_spammer);
				...
				ImGui::Checkbox(crypt_str("Steal player Name"), &g_cfg.misc.namespam);
				if (g_cfg.misc.namespam)
				{
					util::NameSpam(g_cfg.misc.namespam);
				}
...
void c_menu::draw_settings(int child)
{
	...
	ImGui::BeginChild(crypt_str("##CONFIGS__FIRST"), ImVec2(517 * dpi_scale, (child_height - 35) * dpi_scale));
	{
		ImGui::BeginChild(crypt_str("##CONFIGS__FIRST_POSHELNAHUI"), ImVec2(517 * dpi_scale, (child_height - 35) * dpi_scale));
...
void c_menu::draw_legit(int child)
{
	...
	if (!child)
	{
		child_title(crypt_str("LegitBot"));
...
void c_menu::draw_skins(int child)
{
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("whiteless.us"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		auto p = ImGui::GetWindowPos();
		...
		ImGui::BeginChild(crypt_str("##tabs_area"), ImVec2(124 * dpi_scale, 509 * dpi_scale)); // there will be tabs
		{
			...
			ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(10.f * dpi_scale, 10.f * dpi_scale));
			ImGui::SameLine(); ImGui::Text(crypt_str(" whiteless.us "));
			//ImGui::SameLine(); ImGui::Image(yourlogo, ImVec2(85 * dpi_scale, 85 * dpi_scale));
			ImGui::Spacing();
			//  we need some more checks to prevent over-switching tabs
			if (ImGui::MainTab(crypt_str("ragebot"), super_ico, crypt_str("A"), active_tab == 0, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 0;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("antiaim"), super_ico, crypt_str("B"), active_tab == 1, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 1;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("legitbot"), super_ico, crypt_str("H"), active_tab == 6, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 6;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("visuals"), super_ico, crypt_str("C"), active_tab == 2, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 2;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("players"), super_ico, crypt_str("D"), active_tab == 3, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 3;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("misc"), super_ico, crypt_str("E"), active_tab == 4, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 4;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("configs"), super_ico, crypt_str("F"), active_tab == 5, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 5;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("scripts"), super_ico, crypt_str("G"), active_tab == 7, ImVec2(115 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 7;
		...
		// Fill bottom
		ImGui::GetWindowDrawList()->AddRectFilled(ImGui::GetCursorPos() + p, ImGui::GetCursorPos() + p + ImVec2(850 * dpi_scale, ((dpi_scale >= 1.04f && dpi_scale <= 1.07f) ? 2 : 0) + (29 * dpi_scale + (9.45f * (dpi_scale - 1.f)))), ImColor(20, 20, 20, 255), 7.f * dpi_scale, ImDrawCornerFlags_Bot);

		ImGui::PushFont(neverpuk);

		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(850 * dpi_scale - ImGui::CalcTextSize(crypt_str(u8"хули ты смотришь?")).x - (11 * dpi_scale), (16 * dpi_scale) - (ImGui::CalcTextSize(crypt_str(u8"хули ты смотришь?")).y / 2)));

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str(u8"хули ты смотришь?"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
