...
DWORD WINAPI main(PVOID base)
{
	...
	folder = std::string(path) + crypt_str("\\whiteless.us\\");
...
__forceinline void setup_render()
{
	...
	fonts[ESP] = create_font(crypt_str("Raleway"), 11, FW_MEDIUM, FONTFLAG_ANTIALIAS);
	fonts[NAME] = create_font(crypt_str("Raleway"), 15, FW_MEDIUM, FONTFLAG_ANTIALIAS);
	...
	fonts[INDICATORFONT] = create_font(crypt_str("Verdana"), 20, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[DAMAGE_MARKER] = create_font(crypt_str("Verdana"), 17, FW_MEDIUM, FONTFLAG_ANTIALIAS);
...
__forceinline void setup_hooks()
{
	...
	static auto pointer = util::FindSignature(crypt_str("engine.dll"), crypt_str(("55 8B EC 81 EC 64 01 00 00 53 56 57 8B 3D"))); // just the basic. todo: get this better method.
