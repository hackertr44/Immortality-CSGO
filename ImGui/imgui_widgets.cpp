...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec4 text_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 text_hov = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    ...
    if (hovered || active)
    {
        ImVec4 to = active ? text_act : text_hov;
    ...
    static std::map<ImGuiID, float> slide_animation;
    auto it_slider = slide_animation.find(id);
    if (it_slider == slide_animation.end())
    {
        slide_animation.insert({ id, 1.f });
        it_slider = slide_animation.find(id);
    }

    if (active)
    {
        if (it_slider->second < 1.f)
            it_slider->second += deltatime;
        math::clamp(it_slider->second, 0.f, 1.f);
    }
    else
    {
        if (it_slider->second > 1.f)
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 icon_dis = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_hover->second = math::clamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((hovered || clicked) ? 1.f : -1.f)), 0.0f, 1.f);

    const ImVec4 hover_act = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    ...
    RenderFrame(bb.Min, bb.Max, col, true, 2.f * c_menu::get().dpi_scale);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 icon_size = use_icon ? CalcTextSize(icon_label, NULL, true) : ImVec2(0, 0);
    ...
    if (strcmp("Save config", label) != 0 && strcmp("Remove config", label) != 0 && strcmp("Save & Close", label) != 0)
    {
        if (pressed || it_circle->second.active)
        {
            ...
            it_circle->second.alpha = math::clamp(it_circle->second.alpha - (2.f * ImGui::GetIO().DeltaTime), 0.0f, 0.8f);
            it_circle->second.radius = math::clamp(it_circle->second.radius + (240.f * ImGui::GetIO().DeltaTime), 0.0f, 400.f);
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_circle->second.alpha)), 60);
    ...
    if (use_icon)
    {
        PushStyleColor(ImGuiCol_Text, icon_act);

        PushFont(icon_font);
        RenderTextClipped(bb.Min - ImVec2(label_size.x * 1.2f, 0), bb.Max, icon_label, NULL, &icon_size, ImVec2(0.5f, 0.5f), &bb);
        PopFont();
    ...
    RenderTextClipped(bb.Min + (use_icon ? ImVec2(icon_size.x * 1.2f, 0) : ImVec2(0, 0)), bb.Max, label, NULL, &label_size, ImVec2(0.5f, 0.5f), &bb);
    PopStyleColor();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    ImVec4 hover_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    ...
    const ImU32 grab_col = GetColorU32(it_hcolor->second);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 check_col = GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f));
...
/*bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImGui::InvisibleButton(label, ImVec2(width + 190, height));
    ...
    {
        col_bg = ImGui::GetColorU32(ImLerp(ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b()), ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f), t));
        col_bg2 = ImGui::GetColorU32(ImLerp(ImColor(83, 83, 83, 255), ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f), t));
    }
    else
    {
        col_bg = ImGui::GetColorU32(ImLerp(ImColor(5, 5, 5), ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f), t));
        col_bg2 = ImGui::GetColorU32(ImLerp(ImColor(70, 70, 70, 255), ImVec4(g_cfg.menu.menu_theme.r()/ 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b()/ 255.f, 1.f), t));
    }

    window->DrawList->AddRect(ImVec2(p.x + 165, p.y + 8), ImVec2(p.x + 194, p.y + 15), col_bg, height * 0.5f);
    window->DrawList->AddCircleFilled(ImVec2(162 + p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - 2.5f, col_bg2);
    ...
    if (label_size.x > 1.0f)
    {
        ...
        RenderText(ImVec2((p.x + width) - 29, (p.y + height) - 20), label);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        // Draw
        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Max.y),ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b()));
        if (g.LogEnabled)
            LogText(" |");
    }
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b()));
            if (g.LogEnabled)
...
bool ImGui::ListBoxConfigArray(const char* label, int* currIndex, std::vector<std::string>& values, int height)
{
    return ImGui::ListBox(label, currIndex, vector_getter,
        static_cast<void*>(&values), values.size(), height, true);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec4 hover_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(32 / 255.f, 32 / 255.f, 32 / 255.f, 0.4f);;
    ...
    const ImVec4 arrow_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 arrow_dis = ImVec4(150 / 255.f, 150 / 255.f, 150 / 255.f, 1.f);;
    static std::map<ImGuiID, ImVec4> arrow_col_animation;
    auto it_arrow_col = arrow_col_animation.find(id);
    if (it_arrow_col == arrow_col_animation.end())
    {
        arrow_col_animation.insert({ id, hover_dis });
        it_arrow_col = arrow_col_animation.find(id);
    }
    if (popup_open)
    {
        ImVec4 to = arrow_act;
        if (it_arrow_col->second.x != to.x)
        {
            if (it_arrow_col->second.x < to.x)
                it_arrow_col->second.x = ImMin(it_arrow_col->second.x + deltatime, to.x);
            else if (it_arrow_col->second.x > to.x)
                it_arrow_col->second.x = ImMax(to.x, it_arrow_col->second.x - deltatime);
        }

        if (it_arrow_col->second.y != to.y)
        {
            if (it_arrow_col->second.y < to.y)
                it_arrow_col->second.y = ImMin(it_arrow_col->second.y + deltatime, to.y);
            else if (it_arrow_col->second.y > to.y)
                it_arrow_col->second.y = ImMax(to.y, it_arrow_col->second.y - deltatime);
        }

        if (it_arrow_col->second.z != to.z)
        {
            if (it_arrow_col->second.z < to.z)
                it_arrow_col->second.z = ImMin(it_arrow_col->second.z + deltatime, to.z);
            else if (it_arrow_col->second.z > to.z)
                it_arrow_col->second.z = ImMax(to.z, it_arrow_col->second.z - deltatime);
        }
    ...
    static std::map<ImGuiID, arrow_anim> arrow_alpha;
    auto it_arrow_alpha = arrow_alpha.find(id);
    if (it_arrow_alpha == arrow_alpha.end())
    {
        arrow_alpha.insert({ id, { 1.f, false, false, 3 } });
        it_arrow_alpha = arrow_alpha.find(id);
    }
    ...
    if (it_arrow_alpha->second.open != popup_open)
    {
        if (!it_arrow_alpha->second.reverse)
        {
            it_arrow_alpha->second.dir = it_arrow_alpha->second.open ? 2 : 3;

            if (it_arrow_alpha->second.alpha == 0.01f)
                it_arrow_alpha->second.reverse = true;

            it_arrow_alpha->second.alpha = math::clamp(it_arrow_alpha->second.alpha - (8.4f * ImGui::GetIO().DeltaTime), 0.01f, 1.f);
        }
        else
        {
            it_arrow_alpha->second.dir = it_arrow_alpha->second.open ? 3 : 2;

            if (it_arrow_alpha->second.alpha == 1.f)
            {
                it_arrow_alpha->second.reverse = false;
                it_arrow_alpha->second.open = popup_open;
            }

            it_arrow_alpha->second.alpha = math::clamp(it_arrow_alpha->second.alpha + (2.4f * ImGui::GetIO().DeltaTime), 0.01f, 1.f);
        }
    }
    else
    {
        it_arrow_alpha->second.dir = popup_open ? 2 : 3;
        it_arrow_alpha->second.alpha = 1.f;
    }
    ...
    {
        ImU32 text_col = GetColorU32(ImVec4(it_arrow_col->second.x, it_arrow_col->second.y, it_arrow_col->second.z, it_arrow_alpha->second.alpha));
        window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, frame_col, style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
        RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, it_arrow_alpha->second.dir, 1.0f);
    }


    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_hover->second), style.FrameRounding);
    ...
    it_open->second = math::clamp(it_open->second + (3.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.01f, 1.f);


    if (!popup_open)
        it_open->second = 0.01f;
    ...
    PushStyleVar(ImGuiStyleVar_Alpha, it_open->second * c_menu::get().public_alpha);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, ((CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)) * it_open->second)));
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None, current_item))
    {
        return false;
    }
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImVec4 hover_act = ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(74 / 255.f, 79 / 255.f, 143 / 255.f, 0.4f);;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    const ImVec4 hover_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
...
{
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, false, power);
}
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, bool minimum_damage, const char* format)
{
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        g_ctx.globals.focused_on_input = true;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_paste = (is_shortcut_key && IsKeyPressedMap(ImGuiKey_V) && !is_readonly);
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    ImGui::SetCursorPosX(ImGui::GetWindowSize().x - (ImGui::GetWindowSize().x - ImGui::CalcItemWidth()) - 24 * c_menu::get().dpi_scale);
    ...
    if (!config_name.empty() && colors.find(config_name) == colors.end())
    {
        colors[config_name][0] = (float)newcolor->r() / 255.0f;
        colors[config_name][1] = (float)newcolor->g() / 255.0f;
        colors[config_name][2] = (float)newcolor->b() / 255.0f;
        colors[config_name][3] = (float)newcolor->a() / 255.0f;
    }

    // Context menu: display and modify options (before defaults are applied)
    if (!(flags & ImGuiColorEditFlags_NoOptions))
        ColorEditOptionsPopup(colors[config_name], flags);
    ...
    float f[4] = { colors[config_name][0], colors[config_name][1], colors[config_name][2], alpha ? colors[config_name][3] : 1.0f };
    ...
    else if ((flags & ImGuiColorEditFlags_InputRGB) && (flags & ImGuiColorEditFlags_DisplayHSV))
    {
        ...
        if (memcmp(g.ColorEditLastColor, colors[config_name], sizeof(float) * 3) == 0)
        {
            if (f[1] == 0)
    ...
    it_open->second.mult = math::clamp(it_open->second.mult + 3.f + (it_open->second.mult * 0.03f) * GetIO().DeltaTime * (it_open->second.play ? 1.f : -1.f), 0.01f, 1.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const ImVec4 col_v4(colors[config_name][0], colors[config_name][1], colors[config_name][2], alpha ? colors[config_name][3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(24 * c_menu::get().dpi_scale, 17 * c_menu::get().dpi_scale)))
        {
        ...
        if (BeginPopup("picker"))
        {
            ...
            value_changed |= ColorPicker4("##picker", colors[config_name], flags, &g.ColorPickerRef.x);
            EndPopup();
        }
        else
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        colors[config_name][0] = f[0];
        colors[config_name][1] = f[1];
        colors[config_name][2] = f[2];
        if (alpha)
            colors[config_name][3] = f[3];
    }
    ...
    if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget())
    {
        ...
        {
            memcpy((float*)colors[config_name], payload->Data, sizeof(float) * 3); // Preserve alpha if any //-V512
            value_changed = accepted_drag_drop = true;
        }
        if (const ImGuiPayload* payload = AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F))
        {
            memcpy((float*)colors[config_name], payload->Data, sizeof(float) * components);
            value_changed = accepted_drag_drop = true;
        }

        // Drag-drop payloads are always RGB
        if (accepted_drag_drop && (flags & ImGuiColorEditFlags_InputHSV))
            ColorConvertRGBtoHSV(colors[config_name][0], colors[config_name][1], colors[config_name][2], colors[config_name][0], colors[config_name][1], colors[config_name][2]);
        EndDragDropTarget();
    }

    newcolor->SetColor(colors[config_name]);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    SetCursorPosY(GetCursorPosY() - 2 * c_menu::get().dpi_scale);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags__DisplayMask | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;
            SetNextItemWidth(square_sz * 12.0f); // Use 256 + bar sizes?
            value_changed |= ColorPicker4("##picker", col, flags, &g.ColorPickerRef.x);
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4 * c_menu::get().dpi_scale);
        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags__InputMask | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip;
        ColorButton("##current", col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        ImGui::SameLine();
        ImGui::Spacing();

        if (CustomButton("Copy", "##COPY_COLOR", ImVec2(square_sz * 2.57f, 0)))
        {
            memcpy(c_menu::get().color_buffer, col, components * sizeof(float));
        }

        if (CustomButton("Paste", "##COPY_PASTE", ImVec2(square_sz * 2.57f, 0)))
        {
            memcpy(col, c_menu::get().color_buffer, components * sizeof(float));
            value_changed = true;
        }
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    PushStyleColor(ImGuiCol_Text, it_hcolor->second);
    RenderTextClipped(text_min, text_max, label, NULL, &label_size, ImVec2(0.f, 0.5f), &bb_enlarged);
    PopStyleColor();
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, bool custom_selectable)
{
